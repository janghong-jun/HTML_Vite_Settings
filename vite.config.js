...
function scssCompilePlugin() {
  async function compile(scssFile) {
    ...
    try {
      ...
      const cssFile = scssFile.replace(
        /[\\/]scss[\\/](.+)\.scss$/,
        "/css/$1.css",
      );
      ...
      console.log(
        `  [scss] ${slash(relative(SRC, scssFile))} → ${slash(relative(SRC, cssFile))}`,
      );
    } catch (err) {
  ...
  return {
    ...
    handleHotUpdate({ file, server }) {
      ...
      const targets = basename(file).startsWith("_")
        ? glob.sync(`${dirname(file)}/*.scss`)
        : [file];
  ...
  return {
    ...
    async writeBundle() {
      ...
      for (const file of glob.sync(
        "src/**/*.{jpg,jpeg,png,gif,svg,webp,avif,ico}",
      )) {
        if (isIncludePath(file)) continue;
        const rel = relative(SRC, file);
        const dest = resolve(DIST, rel);
